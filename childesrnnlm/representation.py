...
def make_inp_representations(model: RNN,
    ...
    if context_type == 'n':
        ...
        return make_inp_representations_with_context(model, types_eval, prep, exclude_last=False)
    elif context_type == 'm':
        return make_inp_representations_with_context(model, types_eval, prep, exclude_last=True)
    else:
